...
def get_args():
    ...
    if(len(sys.argv) == 4):
        ...
        if(percent > 100 or memory > DEFAULT_MEMORY):
            raise
    #print(type(exec_time), type(proc_num), type(percent), type(memory))
    #print(exec_time, proc_num, percent, memory)
...
def alloc_max_str(memory):
    ...
    while True:
        try:
            a = ' ' * (i * 256 * MEGA)
            (psutil.virtual_memory().used >> 50)
            if((psutil.virtual_memory().used >> 50) > memory):
                break
            del a
        except MemoryError:
            break
